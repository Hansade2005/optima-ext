...
describe("Cline", () => {
    ...
    describe("constructor", () => {
        it("should respect provided settings", () => {
            const cline = new Cline(mockProvider, mockApiConfig, "custom instructions", false, false, 0.95, // 95% threshold
            "test task");
            expect(cline.customInstructions).toBe("custom instructions");
            expect(cline.diffEnabled).toBe(false);
        });
        it("should use default fuzzy match threshold when not provided", () => {
            const cline = new Cline(mockProvider, mockApiConfig, "custom instructions", true, false, undefined, "test task");
        ...
        it("should use provided fuzzy match threshold", () => {
            ...
            const cline = new Cline(mockProvider, mockApiConfig, "custom instructions", true, false, 0.9, // 90% threshold
            "test task");
            expect(cline.diffEnabled).toBe(true);
            expect(cline.diffStrategy).toBeDefined();
        ...
        it("should require either task or historyItem", () => {
            expect(() => {
                new Cline(mockProvider, mockApiConfig, undefined, // customInstructions
                false, // diffEnabled
    ...
    describe("getEnvironmentDetails", () => {
        ...
        describe("API conversation handling", () => {
            it("should clean conversation history before sending to API", async () => {
                ...
                const cleanedMessage = history.find((msg) => msg.content?.some((content) => content.text === "test message"));
            ...
            it("should handle image blocks based on model capabilities", async () => {
                ...
                jest.spyOn(clineWithoutImages, "loadContext").mockImplementation(async (content) => [
                    content,
                    "",
                ]);
                // Set up mock streams
            ...
            it("should handle API retry with countdown", async () => {
                ...
                for (let i = baseDelay; i > 0; i--) {
                    expect(saySpy).toHaveBeenCalledWith("api_req_retry_delayed", expect.stringContaining(`Retrying in ${i} seconds`), undefined, true);
                }
                expect(saySpy).toHaveBeenCalledWith("api_req_retry_delayed", expect.stringContaining("Retrying now"), undefined, false);
            ...
            describe("loadContext", () => {
                it("should process mentions in task and feedback tags", async () => {
                    ...
                    expect(mockParseMentions).toHaveBeenCalledWith("<task>Text with @/some/path in task tags</task>", expect.any(String), expect.any(Object));
                    // Feedback tag content should be processed
                    const toolResult1 = processedContent[2];
                    const content1 = Array.isArray(toolResult1.content) ? toolResult1.content[0] : toolResult1.content;
                    expect(content1.text).toContain("processed:");
                    expect(mockParseMentions).toHaveBeenCalledWith("<feedback>Check @/some/path</feedback>", expect.any(String), expect.any(Object));
