function getEditingInstructions(diffStrategy, experiments) {
    const instructions = [];
    const availableTools = ["write_to_file (for creating new files or complete file rewrites)"];
    ...
    if (experiments?.["insert_content"]) {
        instructions.push("- The insert_content tool adds lines of text to files, such as adding a new function to a JavaScript file or inserting a new route in a Python file. This tool will insert it at the specified line location. It can support multiple operations at once.");
    }
    if (experiments?.["search_and_replace"]) {
...
export function getRulesSection(cwd, supportsComputerUse, diffStrategy, experiments) {
    return `====
