...
export class ClineProvider {
    context;
    outputChannel;
    static sideBarId = "optima-ai.SidebarProvider"; // used in package.json as the view's id
    static tabPanelId = "optima-ai.TabPanelProvider";
    ...
    */
    ...
    async initClineWithTask(task, images) {
        ...
        this.cline = new Cline(this, apiConfiguration, effectiveInstructions, diffEnabled, checkpointsEnabled, fuzzyMatchThreshold, task, images, undefined, experiments);
    }
    async initClineWithHistoryItem(historyItem) {
        ...
        const modePrompt = customModePrompts?.[mode];
        const effectiveInstructions = [globalInstructions, modePrompt?.customInstructions].filter(Boolean).join("\n\n");
        this.cline = new Cline(this, apiConfiguration, effectiveInstructions, diffEnabled, checkpointsEnabled, fuzzyMatchThreshold, undefined, undefined, historyItem, experiments);
    }
            ...
            switch (message.type) {
                ...
                case "enhancePrompt":
                    if (message.text) {
                        try {
                            ...
                            const enhancedPrompt = await singleCompletionHandler(configToUse, supportPrompt.create("ENHANCE", {
                ...
                case "getSystemPrompt":
                    try {
                        ...
                        const diffStrategy = getDiffStrategy(apiConfiguration.apiModelId || apiConfiguration.openRouterModelId || "", fuzzyMatchThreshold, Experiments.isEnabled(experiments, EXPERIMENT_IDS.DIFF_STRATEGY));
                        const cwd = vscode.workspace.workspaceFolders?.map((folder) => folder.uri.fsPath).at(0) || "";
                        const mode = message.mode ?? defaultModeSlug;
                        const customModes = await this.customModesManager.getCustomModes();
                        const systemPrompt = await SYSTEM_PROMPT(this.context, cwd, apiConfiguration.openRouterModelInfo?.supportsComputerUse ?? false, mcpEnabled ? this.mcpHub : undefined, diffStrategy, browserViewportSize ?? "900x600", mode, customModePrompts, customModes, customInstructions, preferredLanguage, diffEnabled, experiments, enableMcpServerCreation);
                        await this.postMessageToWebview({
                            type: "systemPrompt",
                            text: systemPrompt,
                            mode: message.mode,
                ...
                case "updateExperimental": {
                    ...
                    if (message.values[EXPERIMENT_IDS.DIFF_STRATEGY] !== undefined && this.cline) {
                        await this.cline.updateDiffStrategy(Experiments.isEnabled(updatedExperiments, EXPERIMENT_IDS.DIFF_STRATEGY));
                    }
                    await this.postStateToWebview();
                    break;
                }
    ...
    async deleteTaskWithId(id) {
        ...
        if (checkpointsEnabled && baseDir) {
            try {
                ...
                console.error(`[deleteTaskWithId] Error deleting branch ${branch}: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
