...
jest.mock("../../Cline", () => ({
    Cline: jest
        .fn()
        .mockImplementation((provider, apiConfiguration, customInstructions, diffEnabled, fuzzyMatchThreshold, task, taskId) => ({
        abortTask: jest.fn(),
...
describe("ClineProvider", () => {
    ...
    test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
        provider = new ClineProvider({ ...mockContext, extensionMode: vscode.ExtensionMode.Development }, mockOutputChannel);
    ...
    test("uses mode-specific custom instructions in Cline initialization", async () => {
        ...
        expect(Cline).toHaveBeenCalledWith(provider, mockApiConfig, modeCustomInstructions, true, false, 1.0, "Test task", undefined, undefined, experimentDefault);
    });
    ...
    describe("getSystemPrompt", () => {
        ...
        test("uses code mode custom instructions", async () => {
            ...
            const mockAddCustomInstructions = jest.requireMock("../../prompts/sections/custom-instructions")
            ...
            expect(mockAddCustomInstructions).toHaveBeenCalledWith("Code mode specific instructions", "", expect.any(String));
        });
        test("passes diffStrategy and diffEnabled to SYSTEM_PROMPT when previewing", async () => {
            ...
            expect(systemPromptSpy).toHaveBeenCalledWith(expect.anything(), // context
        ...
        test("uses correct mode-specific instructions when mode is specified", async () => {
            ...
            expect(mockAddCustomInstructions).toHaveBeenCalledWith("Architect mode instructions", "", expect.any(String));
        });
    });
    ...
    describe("updateCustomMode", () => {
        test("updates both file and state when updating custom mode", async () => {
            ...
            expect(provider.customModesManager.updateCustomMode).toHaveBeenCalledWith("test-mode", expect.objectContaining({
                slug: "test-mode",
                roleDefinition: "Updated role definition",
            }));
            // Verify state was updated
            expect(mockContext.globalState.update).toHaveBeenCalledWith("customModes", expect.objectContaining({
    ...
    describe("upsertApiConfiguration", () => {
        test("handles error in upsertApiConfiguration gracefully", async () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining("Error create new api configuration"));
        ...
        test("handles buildApiHandler error in updateApiConfiguration", async () => {
            ...
            buildApiHandler.mockImplementationOnce(() => {
