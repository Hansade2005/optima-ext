...
function combineOverlappingMatches(matches, overlapSize = DEFAULT_OVERLAP_SIZE) {
    ...
    for (const match of matches) {
        ...
        const overlapping = matches.filter((m) => Math.abs(m.windowIndex - match.windowIndex) === 1 &&
        ...
        if (overlapping.length > 0) {
            ...
            const avgConfidence = (match.confidence + overlapping.reduce((sum, m) => sum + m.confidence, 0)) / (overlapping.length + 1);
