...
describe("CheckpointService", () => {
    ...
    const initRepo = async ({ baseDir, userName = "Optima AI", userEmail = "support@optima-ai.com", testFileName = "test.txt", textFileContent = "Hello, world!", }) => {
    ...
    describe("saveCheckpoint", () => {
        ...
        it("throws if we're on the wrong branch", async () => {
            ...
            await expect(service.saveCheckpoint("test")).rejects.toThrow(`Git branch mismatch: expected '${service.mainBranch}' but found 'feature'`);
            // Attempt to restore checkpoint from feature branch.
            await expect(service.restoreCheckpoint(service.baseCommitHash)).rejects.toThrow(`Git branch mismatch: expected '${service.mainBranch}' but found 'feature'`);
        });
    ...
    describe("create", () => {
        ...
        it("removes local git config if it matches default and global exists", async () => {
            ...
            jest.spyOn(newGit, "getConfig").mockImplementation((key, scope, callback) => {
                if (scope === "global") {
                    if (key === "user.email") {
                        return Promise.resolve({ value: "global@example.com" });
                    }
                    if (key === "user.name") {
                        return Promise.resolve({ value: "Global User" });
                    }
                }
...
//# sourceMappingURL=CheckpointService.test.js.map