...
suite("Optima AI Task", () => {
    test("Should handle prompt and response correctly", async function () {
        ...
        try {
            ...
            assert.ok(globalThis.provider.messages.some(({ type, text }) => type === "say" && text?.includes("My name is Roo")), "Did not receive expected response containing 'My name is Roo'");
        }
        finally {
        }
    });
